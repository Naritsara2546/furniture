...
$stmt_orders->bind_param("i", $u_id); // Use u_id to fetch orders
$stmt_orders->execute();
...
<head>
    <meta charset="utf-8">
    <title>ประวัติการสั่งซื้อของคุณ</title>
    <style>
        body {
        ...
        <?php if ($rs_orders->num_rows > 0): ?>
            <?php while ($order = $rs_orders->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <a href="orderdetail.php?a=<?php echo $order['oid']; ?>" class="btn">ดูรายละเอียด</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
            <tr>
                <td colspan="4" class="no-data">ไม่มีประวัติการสั่งซื้อ</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
<div style="text-align: center;">
    <a href="home.php" class="btn">กลับ</a>
</div>
</body>
</html>
